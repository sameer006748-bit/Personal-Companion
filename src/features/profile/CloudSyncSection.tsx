...
import { ConflictReview, SyncStatusRow } from './ConflictReview'
...
export function CloudSyncSection() {
  ...
  return <section className="profile-settings-section glass-surface cloud-sync" aria-labelledby="cloud-sync-title">
    ...
    {user && preview ? <><div className="cloud-account"><span><Cloud aria-hidden="true" /></span><div><strong>Connected</strong><small>{maskEmail(preview.email)}</small></div><p>{lastSync ? `Last synced ${new Date(lastSync).toLocaleString()}` : 'Sync not started'}</p></div><div className="cloud-metrics">{([{ label: 'Accounts', value: preview.accounts }, { label: 'Transactions', value: preview.transactions }, { label: 'Receivables', value: preview.receivables }, { label: 'Payables', value: preview.payables }, { label: 'Commitments', value: preview.commitments }]).map((metric) => <div key={metric.label}><strong>{metric.value}</strong><span>{metric.value === 1 ? metric.label.slice(0, -1) : metric.label}</span></div>)}</div>{confirming ? <div className="cloud-sync-confirm"><p>Import these local records to {maskEmail(preview.email)}? Local data will remain on this device.</p><button type="button" className="finance-dialog-save" disabled={isImporting} onClick={() => void importNow()}>{isImporting ? 'Importing' : 'Confirm import'}</button><button type="button" disabled={isImporting} onClick={() => setConfirming(false)}>Cancel</button></div> : <div className="cloud-sync-actions">{!hasSynced ? <button type="button" className="auth-secondary" onClick={() => setConfirming(true)}>Import local data</button> : <span className="cloud-import-complete"><ShieldCheck aria-hidden="true" /> Import complete</span>}<button type="button" className="cloud-sign-out is-destructive" onClick={() => void signOut()}><LogOut aria-hidden="true" /> Sign out</button></div>}
      <SyncStatusRow
        status={sync.status}
      ...
      />
      <ConflictReview conflicts={sync.conflicts} onResolve={sync.resolveConflict} />
    </> : null}
